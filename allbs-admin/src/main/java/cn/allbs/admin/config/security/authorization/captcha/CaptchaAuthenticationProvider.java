package cn.allbs.admin.config.security.authorization.captcha;
...
public class CaptchaAuthenticationProvider extends DaoAuthenticationProvider {
    ...
    public Authentication authenticate(Authentication authentication) throws AuthenticationException {
        ...
        // 获取参数中的验证码
        String code = request.getParameter(CAPTCHA_CODE_NAME);
        if (ObjectUtils.isEmpty(code)) {
            throw new InvalidCaptchaException("The captcha cannot be empty.");
        }

        String captchaId = request.getParameter(CAPTCHA_ID_NAME);
        // 获取缓存中存储的验证码
